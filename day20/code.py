...
def get_all_flips_and_rotations(grid):
    candidates = [grid, grid[::-1], [l[::-1] for l in grid], [l[::-1] for l in grid][::-1]]
    return candidates


def get_all_rotations(grid):
    candidates = [grid]
    last = grid
    for _ in range(3):
...
def get_grid_combintations(grid):
    candidates = list()
    for opt in get_all_flips_and_rotations(grid):
        candidates.extend(get_all_rotations(opt))
    output = []
...
def part1(s):
    tiles = get_tiles_from_input(s)
    tile_options = {num: get_grid_combintations(grid)
    ...
    for p, _ in parts:
        answer *= p
    print(f'Bubu Part one is {answer}')


def superimpose_damn(tile_options, tiling):
    output = []
    for row in tiling:
        grids = []
...
def part2(s):
    ...
    image = superimpose_damn(tile_options, tiling)

    image_opts = get_grid_combintations([list(l) for l in image])

    for opt in image_opts:
        answer = check_mon(opt)
        if answer is not None:
            break

    print(f'Bubu Part two is {answer}')


part1(open('input_test.txt', 'r').read())
part2(open('input_test.txt', 'r').read())
